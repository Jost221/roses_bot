...
class DataType:
    def __init__(self, size=0, nullable=False, default=None, primary_key=False, 
                 check=None, unique=None):
        if primary_key and nullable:
            raise Exception(
                "And so, are you sure you need to make a database? because what you're trying to do reminds me of self-castration. PK must not be null")
        self.size = size
...
class DataTime(DataType):
    name = "TEXT"
    type_r = lambda self, x : datetime.datetime.strptime(x, 
                                                         '%Y-%m-%d %H:%M:%S.%f')
    type_w = str
    inserts = ['"']*2
...
class ForeignKey(DataType):
    name = "REFERENCES"
    def __init__(self, table_column, on_delete=None, on_update=None, 
                 size=0, nullable=False, default=None, primary_key=False, 
                 check=None, unique=None):
